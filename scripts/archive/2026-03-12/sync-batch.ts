...
const datasetId = 'u7yKWD88lMVDAjuPG'

const suburbMap: Record<string, {name: string, postcode: string}> = {
  'vaucluse': { name: 'vaucluse', postcode: '2030' },
  'rose-bay': { name: 'rose bay', postcode: '2029' },
  'bellevue-hill': { name: 'bellevue hill', postcode: '2023' },
}
...
async function sync() {
  ...
  const stats: Record<string, {new: number, updated: number}> = {}
  ...
  for (const item of items) {
    ...
    if (!stats[suburbInfo.name]) stats[suburbInfo.name] = { new: 0, updated: 0 }
  ...
  for (const [suburb, s] of Object.entries(stats)) {
    console.log(`${suburb}: ${s.new} new, ${s.updated} updated`)
  }
  
  // Check final state
  console.log('\n=== Database State ===')
  for (const suburb of ['vaucluse', 'rose bay', 'bellevue hill']) {
